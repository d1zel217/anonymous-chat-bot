...
async def cmd_start(message: types.Message):
    ...
    if user_id not in user_info:
        ...
        await message.answer("Введите ваше имя (от 2 до 30 символов):")
        return
...
@dp.message_handler(lambda message: message.from_user.id not in user_info and len(user_info.get(message.from_user.id, {})) == 0)
...
@dp.message_handler(lambda message: message.from_user.id in user_info and "age" not in user_info[message.from_user.id])
